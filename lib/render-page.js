...
function renderSection(s, i) {
  ...
  // If the section carries an html_override (captured by the editor
  // after inline edits), emit it verbatim instead of re-running the
  // default renderer. This sidesteps the content_patches path-shift
  // problem entirely: the section's HTML travels with the section
  // object in the JSONB, so insert / move / delete just moves it as
  // a unit — no nth-of-type paths to remap, no revert to defaults.
  const inner = (s && s.content && typeof s.content.html_override === "string" && s.content.html_override)
    ? s.content.html_override
    : (key ? SECTION_RENDERERS[key](s.content || {}) : `<!-- unknown section: ${esc(String(s && s.type))} -->`);
...
<head>
  <meta charset="utf-8" />
  <style>html.sm-loading body{visibility:hidden}</style>
  <script>(function(){if(/[?&]edit=1(&|$)/.test(location.search))return;document.documentElement.classList.add("sm-loading");var r=function(){document.documentElement.classList.remove("sm-loading")};window.__SM_REVEAL__=r;window.__SM_PATCHES_PROMISE__=fetch("/api/edits?path="+encodeURIComponent(location.pathname),{cache:"no-store"}).then(function(x){return x.json()}).then(function(d){return(d&&d.patches)||[]}).catch(function(){return[]});setTimeout(r,1500)})();</script>
