...
export async function ensureSchema() {
  ...
  // One-time backfill: any patch still sitting as a draft was likely saved
  // under the old Save/Publish split UX where users didn't realise they had
  // to click Publish separately. Promote them all so nothing lingers as
  // "saved but invisible on the site". Runs each cold start — cheap (only
  // updates rows where published=FALSE, and after the first run there
  // shouldn't be any).
  await sql`UPDATE content_patches SET published = TRUE WHERE published = FALSE`;

  // Uploaded assets (images) kept in Postgres so patches only reference a
  // small URL (/api/asset/N) instead of embedding megabyte base64 data URLs.
  await sql`
  ...
  // Strip editor-only helper classes ("sm-editable-text", "sm-editing", etc.)
  // and contenteditable attributes that leaked into text patches saved before
  // the editor sanitised its output. Idempotent — after the first run there's
  // nothing left to match. Cheap: the WHERE clause skips every clean row.
  await sql`
    UPDATE content_patches
    SET new_content = regexp_replace(
          regexp_replace(new_content, '\\s+contenteditable="[^"]*"', '', 'g'),
          '\\s+class="sm-editable-text"|\\ssm-editable-text|sm-editable-text\\s|sm-editable-text',
          '',
          'g'
        )
    WHERE element_type = 'text' AND new_content ~ 'sm-editable-|contenteditable'
  `;
